...
async def show_faculty_settings(callback: CallbackQuery, state: FSMContext):
    ...
    if current_faculty:
        ...
        faculty_settings_text = get_text(language, "current_faculty").format(faculty=faculty_name)
    else:
        faculty_settings_text = get_text(language, "no_faculty_selected")
    ...
    await callback.message.edit_text(
        faculty_settings_text,
        reply_markup=get_faculty_keyboard(language)
    )

    logger.info(f"Пользователь {user_id} открыл настройки факультета")
...
async def process_faculty_selection(callback: CallbackQuery, state: FSMContext):
    ...
    # Редактируем сообщение, показывая подтверждение выбора и возвращаемся в центр обучения
    await callback.message.edit_text(
        confirmation_text,
...
async def cmd_faculty(message: Message, state: FSMContext):
    ...
    Показывает информацию о текущем факультете и позволяет выбрать новый.
    ...
    if current_faculty:
        # Получаем название факультета на правильном языке
        faculty_name = get_faculty_name(current_faculty, language)
        faculty_text = get_text(language, "current_faculty").format(faculty=faculty_name)
    else:
        faculty_text = get_text(language, "no_faculty_selected")
    ...
    logger.info(f"Пользователь {user_id} запросил информацию о факультете")
...
# Импорт для клавиатуры центра обучения
